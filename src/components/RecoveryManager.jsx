...
export default function RecoveryManager({ admin }) {
    ...
    const fetchRequests = async (showSpinner = true) => {
        ...
        try {
            const res = await fetch(`http://${window.location.hostname}:5001/api/admin/recovery/pending`);
    ...
    const handleApprove = async (requestId) => {
        ...
        setLoading(true);
        try {
            const res = await fetch(`http://${window.location.hostname}:5001/api/admin/recovery/approve`, {
                method: 'POST',
